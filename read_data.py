...
def create_users(connection):
    ...
    connection.execute(
        ...
        SELECT
            "_id"."$oid" AS id,
            active, 
            to_timestamp("createdDate"."$date"/1000) AS creation_timestamp, 
            to_timestamp("lastLogin"."$date"/1000) AS last_login_timestamp, 
            role, 
...
def create_receipts(connection):
    ...
    connection.execute(
        ...
        SELECT
            ...
            to_timestamp("createDate"."$date" / 1000) AS creation_timestamp,
            to_timestamp("dateScanned"."$date" / 1000) AS scan_timestamp,
            to_timestamp("finishedDate"."$date" / 1000) AS finished_timestamp,
            to_timestamp("modifyDate"."$date" / 1000) AS modified_timestamp,
            to_timestamp("pointsAwardedDate"."$date" / 1000) AS points_awarded_timestamp,
